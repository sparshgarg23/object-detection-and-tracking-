...
from lanefit import linefit,tunedfit,final_viz,calc_curve,calc_vehicle_offset
...
def annotate_image(img_in):
	...
	img, abs_bin, mag_bin, dir_bin, hls_bin = combinedthresh(undist)
	binary_warped, binary_unwarped, m, m_inv = perspective_transform(img)

	# Perform polynomial fit
	if not detected:
		# Slow line fit
		ret = linefit(binary_warped)
		...
		ret = tunedfit(binary_warped, left_fit, right_fit)
		left_fit = ret['left_fit']
		right_fit = ret['right_fit']
		nonzerox = ret['nonzerox']
		nonzeroy = ret['nonzeroy']
...
if __name__ == '__main__':
	# Annotate the video
	annotate_video('project_video_result.mp4', 'out.mp4')
