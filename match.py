...
class BoxMatcher(object):
    ...
    def match_idx_of_box1_idx(self, box1_idx):
        """
        # Args
            box1_idx(detected box index) : int
        
        # Returns
            box2_idx(tracked box index) : int or None
    ...
    def match_idx_of_box2_idx(self, box2_idx):
        """
        # Args
            box2_idx(tracked box index) : int
         
        # Returns
            box1_idx(corresponding detected box) : int or None
                if matching index does not exist, return None
            iou : float
    ...
    def _calc_maximun_ious(self):
        ious_gt = self._calc(self._boxes1, self._boxes2)
        ious = np.max(ious_gt, axis=0)
        return ious
